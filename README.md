...
### 🏗 2. 平台调用 `batchSafeFreeMint` 批量铸造流程

```mermaid
graph TD
    A["🟩 前端：用户付款成功"] --> B["🟩 前端：构造 MintRequest[]"]
    B --> C["🟩 前端：调用 batchSafeFreeMint()"]
    C --> D["🟦 合约：逐个调用 _safeFreeMint()"]
    D --> E["🟦 合约：记录成功/失败，发出 BatchFreeMinted 事件"]
    E --> F["🟩 前端：监听事件，更新铸造记录 & 提示用户"]
```

---

### 🧾 3. 平台调用 `freeMint` 单独铸造流程
...
graph TD
    ...
    B --> C["🟩 前端：调用 freeMint()"]
    C --> D["🟦 合约：验证并调用 _mint()"]
    D --> E["🟦 合约：发出 ChapterMinted 事件"]
    E --> F["🟩 前端：监听事件，提示用户铸造成功"]
```
...
graph TD
    ...
    D -- 否 --> J["🟩 平台：等待"]
    J --> K["⏰ 超过 7 天后自动调用 investorRegistration()"]
    K --> L{"合约验证是否持有？"}
    L -- 是 --> M["🟩 更新为成功，付款创作者"]
    L -- 否 --> N["🟩 退款投资者，记录失败"]
```
